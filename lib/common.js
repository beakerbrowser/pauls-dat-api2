...
function toBeakerError (err, info) {
  if (err.code == 16) {
    ...
    return new Error('The Hyperspace daemon has reset due to internal error. Please try again.')
  } else if (err.toString().indexOf('Block not available') !== -1) {
    return new NotAvailableError()
  } else if (err.toString().indexOf('RPC stream destroyed') !== -1) {
    return new Error('The Hyperspace daemon has shut down.')
