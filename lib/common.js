...
const {NotFoundError, EntryAlreadyExistsError, DestDirectoryNotEmpty, ArchiveNotWritableError} = require('beaker-error-constants')
...
async function ensureParentDir (archive, name) {
  ...
  for (let i = parts.length - 1; i > 0; i--) {
    ...
    let st = await archive.stat(path).catch(e => undefined)
    if (st) return
