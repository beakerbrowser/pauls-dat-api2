...
const {maybe, encodeMetadata} = require('./common')
...
function exportFilesystemToArchive (opts, cb) {
  return maybe(cb, async function () {
    ...
    var progressCb = typeof opts.progress === 'function' ? opts.progress : undefined
    ...
    if (dstStat && !srcStat.isDirectory() && dstStat.isDirectory()) {
      dstPath = path.join(dstPath, path.basename(srcPath))
    }
...
function exportArchiveToArchive (opts, cb) {
  return maybe(cb, async function () {
    ...
    var ignore = Array.isArray(opts.ignore) ? opts.ignore : DEFAULT_IGNORE
    var dryRun = opts.dryRun === true
    var intoTargetFolder = opts.intoTargetFolder === true
    ...
    const statThenExport = async function (srcPath, dstPath) {
      ...
      var srcStat = await stat(srcArchive, srcPath)
      if (srcStat.mount) {
        await exportMount(srcPath, srcStat, dstPath)
    ...
    const exportMount = async function (srcPath, srcStat, dstPath) {
      // make sure the destination is available
      var dstStat
      try { dstStat = await stat(dstArchive, dstPath) } catch (e) {}
      if (dstStat) {
        if (dstStat.mount) {
          if (dstStat.mount.key === srcStat.mount.key) return // all set
          if (!dryRun) await unmount(dstArchive, dstPath)
          stats.removedFolders.push(dstPath)
        } else if (dstStat.isFile()) {
          stats.removedFiles.push(dstPath)
          if (!dryRun) await unlink(dstArchive, dstPath)
        } else {
          if (!dryRun) await rmdir(dstArchive, dstPath, {recursive: true})
          stats.removedFolders.push(dstPath)
        }
      ...
      // write the mount
      if (!dryRun) await mount(dstArchive, dstPath, srcStat.mount.key)
      stats.addedFolders.push(dstPath)
    }
    ...
    const exportFile = async function (srcPath, srcStat, dstPath) {
      ...
      // write the file
      encodeMetadata(srcStat.metadata)
      if (dryRun) return
      return new Promise((resolve, reject) => {
        pump(
          srcArchive.createReadStream(srcPath),
          dstArchive.createWriteStream(dstPath, {metadata: srcStat.metadata}),
          err => {
            if (err) reject(err)
            else resolve()
          }
