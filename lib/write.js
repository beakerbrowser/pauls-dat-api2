...
const {maybe, toBeakerError, toValidEncoding, encodeMetadata} = require('./common')
...
function writeFile (archive, name, data, opts, cb) {
  ...
  return maybe(cb, async function () {
    ...
    if (parentName !== '/' && parentName !== '.') {
      if (opts && opts.ensureParent) {
        try {
          await mkdir(archive, parentName, {ensureParent: true})
        } catch (e) {
          if (e.name !== 'EntryAlreadyExistsError') {
            throw e
          }
        }
      }
